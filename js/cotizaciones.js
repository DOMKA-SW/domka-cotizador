...
form.addEventListener("submit", async (e) => {
  ...
  // Generar token criptográfico antes de guardar
  const tokenPublico = generarTokenSeguro();
  cotizacion.tokenPublico = tokenPublico;

  const docRef = await db.collection("cotizaciones").add(cotizacion);

  const linkPublico = generarLinkPublico("cotizacion", docRef.id, tokenPublico);
