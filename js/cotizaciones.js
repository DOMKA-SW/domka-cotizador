...
form.addEventListener("submit", async (e) => {
  ...
  // Generar número de secuencia (COT-XXXX)
  const numeroCot = await generarNumero('COT');
  ...
  // Guardar cotización (sin tokenPublico en el documento)
  const docRef = await db.collection("cotizaciones").add(cotizacion);

  // Crear token seguro en colección separada public_tokens
  try {
    await crearTokenPublico(docRef.id, "cotizaciones");
  } catch (err) {
    console.error("⚠️ Token público no creado:", err.message);
  }
...
async function cargarCotizaciones() {
  ...
  snap.forEach(doc => {
    ...
    tr.innerHTML = `
      <td class="p-2">
        <span class="font-mono text-xs text-green-700 font-semibold">${c.numero || '—'}</span>
      </td>
      <td class="p-2">
        ${nombreCliente}
