  ...
  snapshot.forEach((doc) => {
    ...
    listaClientes.innerHTML += `
      <tr class="border-b table-row-striped">
        ...
        <td class="p-2">
          <span style="font-family:monospace;font-size:.68rem;background:#f0f0f0;padding:2px 6px;border-radius:4px;cursor:pointer;user-select:all"
            title="Copiar ID de Firestore"
            onclick="navigator.clipboard.writeText('${doc.id}').then(()=>alert('ID copiado: ${doc.id}'))"
          >${doc.id.substring(0,8)}…</span>
        </td>
